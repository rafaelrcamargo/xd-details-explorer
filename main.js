...
function styleToWeight(fontStyle) {
  if (fontStyle.match(/\bBold\b/i)) {
    return "bold"
  } else if (fontStyle.match(/\bBlack\b/i) || fontStyle.match(/\bHeavy\b/i)) {
    // TODO: "extra bold"? (move precedence higher if so)
    return 900
...
function copy(selection) {
  ...
  if (node.hasRoundedCorners) {
    ...
    if (tlbr && trbl) {
      ...
      css += `border-radius: ${num(corners.topLeft)}px ${num(corners.topRight)}px ${num(corners.bottomRight)}px ${num(
        corners.bottomLeft
      )}px;\n`
    }
  }
  ...
  if (node.blur && node.blur.visible) {
    ...
    if (blur.isBackgroundEffect) {
      ...
      if (blur.fillOpacity > 0) {
        ...
        css += `/* (plus shape's fill blended on top as a separate layer with ${num(
          blur.fillOpacity * 100
        )}% opacity) */\n`
      }
    } else {
